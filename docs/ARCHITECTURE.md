...
11. **Sollstunden-Warnung SW1–SW12** (seit v3.9.78) — warnt optional, wenn der Monats-Saldo und/oder der Gleitzeitkonto-Saldo um mindestens eine eingestellte Schwelle (10/20/30 % oder eigener Wert) über oder unter dem jeweiligen Soll liegt. Reine Berechnung in `modules/soll-warning.js` (`evaluateSollWarning`, `computeActiveSollWarnings`, Mindest-Soll-Guard `MIN_TARGET_MIN_FOR_EVALUATION`), Banner-Builder in `modules/render/soll-warning-banner.js`, App-Integration via `updateSollWarningBanner()`/`computeBalanceWarningInfo()` in `app.js`. Anzeige: Banner in der Erfassen-Ansicht (Direktlink zum Gleitzeitkonto, 7-Tage-Snooze) plus Hervorhebung der Saldo-Kachel (`sw-flagged`) im Monats- und Gleitzeitkonto-Tab. Beide Bausteine sind standardmäßig deaktiviert, getrennt einstellbar und im Freiberufler-Modus vollständig ausgeblendet (kein Soll/Saldo-Konzept dort); ehemalige Arbeitgeber werden nicht bewertet. Siehe `runSollWarningUnits` in `scripts/regression.mjs`.
