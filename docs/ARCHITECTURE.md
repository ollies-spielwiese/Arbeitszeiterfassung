...
11. **Sollstunden-Warnung SW1–SW19** (seit v3.9.78, Banner seit v3.9.79 zusätzlich in der Übersicht, anteiliges Soll + Mindest-Arbeitstage-Schutz seit v3.9.80) — warnt optional, wenn der Monats-Saldo und/oder der Gleitzeitkonto-Saldo um mindestens eine eingestellte Schwelle (10/20/30 % oder eigener Wert) über oder unter dem jeweiligen Soll liegt. Reine Berechnung in `modules/soll-warning.js` (`evaluateSollWarning`, `computeActiveSollWarnings`, Mindest-Soll-Guard `MIN_TARGET_MIN_FOR_EVALUATION`), Banner-Builder in `modules/render/soll-warning-banner.js`, App-Integration via `updateSollWarningBanner()`/`computeBalanceWarningInfo()` in `app.js`. Anzeige: identisches Banner in Erfassen- **und** Übersicht-Ansicht (`SOLL_WARNING_BANNER_TARGETS` in `app.js` aktualisiert beide DOM-Instanzen gemeinsam; Direktlink zum Gleitzeitkonto, global geteilter 7-Tage-Snooze) plus Hervorhebung der Saldo-Kachel (`sw-flagged`) im Monats- und Gleitzeitkonto-Tab. Beide Bausteine sind standardmäßig deaktiviert, getrennt einstellbar und im Freiberufler-Modus vollständig ausgeblendet (kein Soll/Saldo-Konzept dort); ehemalige Arbeitgeber werden nicht bewertet.
   - **Nur der proaktive Übersicht/Erfassen-Banner für den Monats-Baustein** (nicht das Gleitzeitkonto-Banner, nicht die manuelle Kachel-Hervorhebung `computeBalanceWarningInfo()` für frei gewählte Monate im Monat-Bericht) rechnet seit v3.9.80 mit dem **anteiligen Soll des bereits vergangenen Monats** statt dem vollen Monatssoll: `computeElapsedMonthProgress()` in `modules/compute.js` liefert `elapsedWorkdays`/`totalWorkdays`/`proratedTargetMin` (bei `hoursMode='week'` exakte Tagsumme über die vergangenen Soll-Arbeitstage, sonst Verhältnis `voll × elapsedWorkdays/totalWorkdays`).
   - **Mindest-Arbeitstage-Schutz**: Die Warnung wird erst ausgewertet, wenn mindestens `computeMinElapsedWorkdaysForThreshold(effectiveThresholdPct)` Arbeitstage im Monat vergangen sind (`= floor(100/effectiveThresholdPct) + 1`) — ein einzelner fehlender Arbeitstag am Monatsanfang kann die Schwelle so rechnerisch nicht mehr allein überschreiten.
   - **Prozent-Mindestschwelle**: `clampMonthThresholdPct()` wertet einen eingestellten Schwellenwert unter 10 % intern nie niedriger als 10 % (das UI-Eingabefeld `setting-sw-month-threshold-custom` hat entsprechend `min="10"`; das Gleitzeitkonto-Feld bleibt bei `min="1"`), begrenzt damit `minDays` nach oben und ist in den Einstellungen als Hinweistext (`#sw-month-threshold-hint`) dokumentiert.
   - **Betrags-Mindestwert**: `MIN_ABSOLUTE_DEVIATION_MIN` (30 Minuten) unterdrückt zusätzlich jede Abweichung, deren absoluter Betrag darunter liegt, selbst wenn die Prozent-Schwelle rein rechnerisch erreicht wäre (schützt vor Rundungs-/Bagatell-Fehlalarmen bei kleinen Sollwerten).
   - Siehe `runSollWarningUnits` in `scripts/regression.mjs` (SW1–SW14 Basisverhalten inkl. Banner/Snooze/ehemalige Arbeitgeber, SW15–SW19 Mindest-Arbeitstage-Schutz, Prozent-Mindestschwelle, Betrags-Mindestwert und Hinweistext).
