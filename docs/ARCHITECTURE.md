...
11. **Sollstunden-Warnung SW1–SW14** (seit v3.9.78, Banner seit v3.9.79 zusätzlich in der Übersicht) — warnt optional, wenn der Monats-Saldo und/oder der Gleitzeitkonto-Saldo um mindestens eine eingestellte Schwelle (10/20/30 % oder eigener Wert) über oder unter dem jeweiligen Soll liegt. Reine Berechnung in `modules/soll-warning.js` (`evaluateSollWarning`, `computeActiveSollWarnings`, Mindest-Soll-Guard `MIN_TARGET_MIN_FOR_EVALUATION`), Banner-Builder in `modules/render/soll-warning-banner.js`, App-Integration via `updateSollWarningBanner()`/`computeBalanceWarningInfo()` in `app.js`. Anzeige: identisches Banner in Erfassen- **und** Übersicht-Ansicht (`SOLL_WARNING_BANNER_TARGETS` in `app.js` aktualisiert beide DOM-Instanzen gemeinsam; Direktlink zum Gleitzeitkonto, global geteilter 7-Tage-Snooze) plus Hervorhebung der Saldo-Kachel (`sw-flagged`) im Monats- und Gleitzeitkonto-Tab. Beide Bausteine sind standardmäßig deaktiviert, getrennt einstellbar und im Freiberufler-Modus vollständig ausgeblendet (kein Soll/Saldo-Konzept dort); ehemalige Arbeitgeber werden nicht bewertet. Siehe `runSollWarningUnits` in `scripts/regression.mjs`.
